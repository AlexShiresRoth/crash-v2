...
const Nav = ({ navigation, logo }: Props) => {
  return (
    <nav className="w-full flex justify-center items-center border-b-[1px] border-slate-500 bg-black sticky top-0 z-50">
      <div className="w-3/4 flex gap-4 items-center">
        ...
        {navigation?.navigationItemsCollection?.items?.map((item) => (
          <Link
            href={item?.slug}
            key={item?.sys?.id}
            className="text-slate-500 text-sm hover:text-slate-50 transition-all p-2 hover:bg-slate-500/30 rounded"
          >
            {item?.displayTitle}
          </Link>
        ))}
